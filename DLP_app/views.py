...
from .utils import search_for_leaks, send_message_to_SQS
import boto3
    ...
    if 'event' in json_dict:
        ...
        if event_msg['type'] == 'message':
            if "files" in event_msg:
                # search_for_leaks(event_msg["files"][0]["preview"])
                send_message_to_SQS(event_msg["files"][0]["preview"],event_msg["client_msg_id"])
                return HttpResponse(status=201)
            elif "text" in event_msg:
                # search_for_leaks(event_msg["text"])
                send_message_to_SQS(event_msg["text"],event_msg["client_msg_id"])
